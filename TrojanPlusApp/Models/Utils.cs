...
namespace TrojanPlusApp.Models
{
    public static class Utils
    {
        ...
        public static string PrepareConfig(HostModel host, HostsViewModel hosts)
        {
            string config = ConfigTemplate;

            config = config.Replace("${run_type}", "client_tun");
            config = config.Replace("${remote_addr}", host.HostAddress);
            config = config.Replace("${remote_port}", host.HostPort.ToString());
            config = config.Replace("${password}", host.Password);

            config = config.Replace("${udp_timeout}", "10");
            config = config.Replace("${udp_socket_buf}", "16384");
            config = config.Replace("${udp_recv_buf}", "4096");

            config = config.Replace("${log_level}", "5"); // close log

            config = config.Replace("${ssl.verify}", host.SSLVerify.ToLowerString());
            config = config.Replace("${ssl.verify_hostname}", host.SSLVerify.ToLowerString());

            config = config.Replace("${tcp.fast_open}", "true");
            config = config.Replace("${tcp.connect_time_out}", "5");

            config = config.Replace("${experimental.pipeline_num}", host.EnablePipeline ? "5" : "0");
            ...
            config = config.Replace("${route.proxy_type}", ((int)host.Route).ToString());
